...
size_t slash_to_tilde(size_t rand)
{
    /* For safe filenames on Linux. */
    return rand == '/' ? '~' : rand;
}

#define PRINTABLE_MIN      0x21
#define PRINTABLE_MAX      0x7d
...
int main(int argc, char **argv)
{
    ...
    default:
        fprintf(stderr, "Usage:\n\t"
                "rpg-linux-filename [min-length] max-length [how-many]\n");
        exit(1);
        break;
    }
